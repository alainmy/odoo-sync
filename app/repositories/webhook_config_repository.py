...
class WebhookConfigRepository:
    ...
    def create(self, webhook_data: WebhookConfigCreate) -> Optional[WebhookConfig]:
        ...
        try:
            ...
            logger.info(
                f"Created webhook config {webhook.id} for instance {webhook.instance_id}")
            return webhook
    ...
    def update_wc_webhook_id(self, webhook_id: int, wc_webhook_id: int) -> Optional[WebhookConfig]:
        ...
        try:
            ...
            self.db.refresh(webhook)
            logger.info(
                f"Updated webhook {webhook_id} with WC webhook ID {wc_webhook_id}")
            return webhook
        except Exception as e:
    ...
    def update_delivery_metrics(self, webhook_id: int) -> Optional[WebhookConfig]:
        ...
        try:
            ...
            logger.error(
                f"Error updating delivery metrics for {webhook_id}: {e}")
            self.db.rollback()
            return None
