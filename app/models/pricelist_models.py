...
class PricelistSync(Base):
    ...
    odoo_pricelist_id = Column(Integer, nullable=False, comment="Odoo pricelist ID")
    odoo_pricelist_name = Column(String(255), nullable=True, comment="Odoo pricelist name")
    instance_id = Column(Integer, nullable=False, comment="WooCommerce instance ID")
    active = Column(Boolean, default=True, nullable=False, comment="Is this pricelist active for sync")
    price_type = Column(String(50), default='regular', nullable=False, comment="Type: regular, sale, meta")
    meta_key = Column(String(100), nullable=True, comment="Meta key for custom price fields")
    last_synced_at = Column(DateTime, nullable=True, comment="Last sync timestamp")
    created_at = Column(DateTime, server_default=func.now(), nullable=False)
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now(), nullable=False)
    message = Column(Text, nullable=True, comment="Last sync message or errors")
    
    # Unique constraint
    __table_args__ = (
        UniqueConstraint('odoo_pricelist_id', 'instance_id', name='uq_pricelist_sync_instance'),
