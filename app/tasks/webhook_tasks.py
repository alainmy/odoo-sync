...
def process_webhook(
    ...
    try:
        logger.info(
            f"Processing webhook: {event_type} (ID: {event_id}) for instance {instance_id}")
        ...
        if existing_log:
            if existing_log.status == "completed":
                ...
                logger.warning(
                    f"Webhook {event_id} is being processed by another worker")
                return {
                    "success": False,
                    "action": "duplicate",
        ...
        if event_type in ["product.created", "product.created", "product.updated", "product.update", "product.deleted"]:
            ...
            logger.info(
                f"Queued product sync task {task_result.id} for product {payload.get('id')}")
            result = {"success": True, "action": "queued",
            ...
            logger.info(
                f"Queued order sync task {task_result.id} for order {payload.get('id')}")
            result = {"success": True, "action": "queued",
                      "task_id": task_result.id}
        ...
        webhook_log.error_message = result.get(
            "error") if not result.get("success") else None
        self.db.commit()
