...
def update_webhook_config(
    ...
    update_data.delivery_url = settings.fast_api_host + \
        f"/api/v1/webhook-receiver/wc/{webhook.instance_id}/{webhook.topic}"
    ...
    if update_in_wc and updated_webhook.wc_webhook_id:
        try:
            ...
            if instance:
                ...
                if wc_response:
                    logger.info(
                        f"Updated webhook {updated_webhook.wc_webhook_id} status in WooCommerce")
                else:
                    logger.warning(
                        f"Webhook updated locally but failed to update in WooCommerce")
    ...
    if delete_from_wc and webhook.wc_webhook_id:
        try:
            ...
            if instance:
                ...
                wcapi = create_wc_api_client(wc_config)
                service = WebhookService(db)
                service.delete_webhook_in_woocommerce(
                    wcapi, webhook.wc_webhook_id)
        except Exception as e:
